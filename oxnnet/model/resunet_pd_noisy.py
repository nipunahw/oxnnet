...
class Model(object):
    ...
    def build_net(self, X, PD, reuse=False, segment_size_in=segment_size_in,feats=None):
        ...
        with tf.variable_scope("level1", reuse=reuse):
            net1_bmode = tflearn.layers.conv_3d(X, 8, 3, padding='valid',regularizer='L2',reuse=reuse,scope='conv1-1-bmode', weights_init=init)
            net1_pd = tflearn.layers.conv_3d(PD, 8, 3, padding='valid',regularizer='L2',reuse=reuse,scope='conv1-1-pd', weights_init=init)
        ...
        with tf.variable_scope("level2", reuse=reuse):
            net2_in = tflearn.layers.conv_3d(net1, 32, 2, strides=2, padding='valid',regularizer='L2',reuse=reuse,scope='ds1', weights_init=init)
            net2_in = tflearn.layers.normalization.batch_normalization(net2_in, reuse=reuse, scope='batch2-1')
            net2 = tflearn.layers.conv_3d(net2_in, 32, 3, padding='valid', regularizer='L2',reuse=reuse,scope='conv2-1', weights_init=init)
        ...
        with tf.variable_scope("level3"):
            net3_in = tflearn.layers.conv_3d(net2, 64, 2, strides=2, padding='valid',regularizer='L2',reuse=reuse,scope='ds2', weights_init=init)
            net3_in = tflearn.layers.normalization.batch_normalization(net3_in, reuse=reuse, scope='batch3-1')
            net3 = tflearn.layers.conv_3d(net3_in, 64, 3, padding='valid',regularizer='L2',reuse=reuse,scope='conv3-1', weights_init=init)
        ...
        with tf.variable_scope("level4"):
            net4_in = tflearn.layers.conv_3d(net3, 128, 2, strides=2, padding='valid',regularizer='L2',reuse=reuse,scope='ds3', weights_init=init)
            net4_in = tflearn.layers.normalization.batch_normalization(net4_in, reuse=reuse, scope='batch4-1')
            net4 = tflearn.layers.conv_3d(net4_in, 128, 3, padding='valid',regularizer='L2',reuse=reuse,scope='conv4-1', weights_init=init)
        ...
        with tf.variable_scope("level5"):
            net5 = tflearn.layers.conv.conv_3d_transpose(net4, 64, 2, [12]*3,strides=2, padding='same',regularizer='L2',reuse=reuse,scope='trans2', weights_init=init)
            net5 = tflearn.layers.merge_ops.merge([net5, tf.slice(net3,[0]+[(16-12)//2]*3+[0],[-1]+[12]*3+[64])],'elemwise_sum',name='merge5')
            net5 = tflearn.layers.conv_3d(net5, 64, 3, padding='valid',regularizer='L2',reuse=reuse,scope='conv5-1', weights_init=init)
            net5 = tflearn.layers.normalization.batch_normalization(net5, reuse=reuse, scope='batch5-2')
            net5 = NTanh(net5, tf.constant(True, dtype=tf.bool))

        with tf.variable_scope("level6"):
            net6 = tflearn.layers.conv.conv_3d_transpose(net5, 32, 2, [20]*3,strides=2, padding='same',regularizer='L2',reuse=reuse,scope='trans3', weights_init=init)
            ...
            net6 = tflearn.layers.conv_3d(net6, 32, 3, padding='valid',regularizer='L2',reuse=reuse,scope='conv6-1', weights_init=init)
            net6 = tflearn.layers.normalization.batch_normalization(net6, reuse=reuse, scope='batch6-2')
            net6 = NTanh(net6, tf.constant(True, dtype=tf.bool))

        with tf.variable_scope("level7"):
            net7 = tflearn.layers.conv.conv_3d_transpose(net6, 16, 2, [36]*3,strides=2, padding='same',regularizer='L2',reuse=reuse,scope='trans4', weights_init=init)
            ...
            net7 = tflearn.layers.conv_3d(net7, 16, 3, padding='valid',regularizer='L2',reuse=reuse,scope='conv9-1', weights_init=init)
            net7 = tflearn.layers.normalization.batch_normalization(net7, reuse=reuse, scope='batch7-2')
            net7 = NTanh(net7, tf.constant(True, dtype=tf.bool))

        with tf.variable_scope("out"):
            net_fc1 = tflearn.layers.conv_3d(net7, 16, 1, padding='valid',regularizer='L2',reuse=reuse,scope='fc1', weights_init=init)
            ...
            net_out = tflearn.layers.conv_3d(net_fc1, 2, 1, padding='valid',regularizer='L2',reuse=reuse,scope='output',weights_init=init)
