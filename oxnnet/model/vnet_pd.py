...
class Model(object):
    ...
    def build_net(self, X, PD, reuse=False):
        ...
        with tf.variable_scope("level1", reuse=reuse):
            net1_bmode = tflearn.layers.conv_3d(X, 4, 3, activation='prelu',padding='same',regularizer='L2',reuse=reuse,scope='conv1-1-bmode', weights_init=init)
            net1_pd = tflearn.layers.conv_3d(PD, 4, 3, activation='prelu',padding='same',regularizer='L2',reuse=reuse,scope='conv1-1-pd', weights_init=init)
