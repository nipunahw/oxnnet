...
class Model(object):
    ...
    def build_net(self, X, reuse=False):
        ...
        with tf.variable_scope("level1", reuse=reuse):
            net1 = tflearn.layers.conv_3d(X, 16, 3, activation='linear', padding='valid', regularizer='L2', reuse=reuse,
                                          scope='conv1-1', weights_init=init)
            net1 = tflearn.activation(net1, 'prelu')

        with tf.variable_scope("level2", reuse=reuse):
            net2_in = tflearn.layers.conv_3d(net1, 32, 2, strides=2, activation='prelu', padding='valid',
                                             regularizer='L2', reuse=reuse, scope='ds1', weights_init=init)
            net2 = tflearn.layers.conv_3d(net2_in, 32, 3, activation='linear', padding='valid', regularizer='L2',
                                          reuse=reuse, scope='conv2-1', weights_init=init)
            net2 = tflearn.activation(net2, 'prelu')

        with tf.variable_scope("level1_2", reuse=reuse):
            net1_2 = tflearn.layers.conv.conv_3d_transpose(net2, 16, 2, [36] * 3, strides=2, activation='prelu',
                                                           padding='same', regularizer='L2', reuse=reuse,
                                                           scope='trans1', weights_init=init)
            net1_2 = tflearn.layers.merge_ops.merge(
                [net1_2, tf.slice(net1, [0] + [(74 - 36) // 2] * 3 + [0], [-1] + [36] * 3 + [16])], 'elemwise_sum',
                name='merge1')
            net1_2 = tflearn.layers.conv_3d(net1_2, 16, 3, activation='linear', padding='valid', regularizer='L2',
                                            reuse=reuse,
                                            scope='conv1-2', weights_init=init)
            net1_2 = tflearn.activation(net1_2, 'prelu')

        with tf.variable_scope("level3"):
            net3_in = tflearn.layers.conv_3d(net2, 64, 2, strides=2, activation='prelu', padding='valid',
                                             regularizer='L2', reuse=reuse, scope='ds2', weights_init=init)
            net3 = tflearn.layers.conv_3d(net3_in, 64, 3, activation='linear', padding='valid', regularizer='L2',
                                          reuse=reuse, scope='conv3-1', weights_init=init)
            net3 = tflearn.activation(net3, 'prelu')

        with tf.variable_scope("level2_2", reuse=reuse):
            net2_2 = tflearn.layers.conv.conv_3d_transpose(net2, 32, 2, [20] * 3, strides=2, activation='prelu',
                                                           padding='same', regularizer='L2', reuse=reuse,
                                                           scope='trans2',
                                                           weights_init=init)
            net2_2 = tflearn.layers.merge_ops.merge(
                [net2_2, tf.slice(net2, [0] + [(36 - 20) // 2] * 3 + [0], [-1] + [20] * 3 + [32])], 'elemwise_sum',
                name='merge2')
            net2_2 = tflearn.layers.conv_3d(net2_2, 32, 3, activation='linear', padding='valid', regularizer='L2',
                                            reuse=reuse, scope='conv2-2', weights_init=init)
            net2_2 = tflearn.activation(net2_2, 'prelu')

        with tf.variable_scope("level1_3", reuse=reuse):
            net1_3 = tflearn.layers.conv.conv_3d_transpose(net2_2, 16, 2, [36] * 3, strides=2, activation='prelu',
                                                           padding='same', regularizer='L2', reuse=reuse,
                                                           scope='trans3',
                                                           weights_init=init)
            net1_3 = tflearn.layers.merge_ops.merge(
                [net1_3, tf.slice(net1, [0] + [(74 - 36) // 2] * 3 + [0], [-1] + [36] * 3 + [16]),
                 tf.slice(net1_2, [0] + [(74 - 36) // 2] * 3 + [0], [-1] + [36] * 3 + [16])], 'elemwise_sum',
                name='merge3')
            net1_3 = tflearn.layers.conv_3d(net1_3, 16, 3, activation='linear', padding='valid', regularizer='L2',
                                            reuse=reuse, scope='conv1-3', weights_init=init)
            net1_3 = tflearn.activation(net1_3, 'prelu')

        with tf.variable_scope("level4"):
            net4_in = tflearn.layers.conv_3d(net3, 128, 2, strides=2, activation='prelu', padding='valid',
                                             regularizer='L2', reuse=reuse, scope='ds3', weights_init=init)
            net4 = tflearn.layers.conv_3d(net4_in, 128, 3, activation='linear', padding='valid', regularizer='L2',
                                          reuse=reuse, scope='conv4-1', weights_init=init)
            net4 = tflearn.activation(net4, 'prelu')

        with tf.variable_scope("level5"):
            net5 = tflearn.layers.conv.conv_3d_transpose(net4, 64, 2, [12] * 3, strides=2, activation='prelu',
                                                         padding='same', regularizer='L2', reuse=reuse, scope='trans4',
                                                         weights_init=init)
            net5 = tflearn.layers.merge_ops.merge([net5,
                                                   tf.slice(net3, [0] + [(16 - 12) // 2] * 3 + [0],
                                                            [-1] + [12] * 3 + [64])], 'elemwise_sum', name='merge4')
            net5 = tflearn.layers.conv_3d(net5, 64, 3, activation='linear', padding='valid', regularizer='L2',
                                          reuse=reuse, scope='conv5-1', weights_init=init)
            net5 = tflearn.activation(net5, 'prelu')

        with tf.variable_scope("level6"):
            net6 = tflearn.layers.conv.conv_3d_transpose(net5, 32, 2, [20] * 3, strides=2, activation='prelu',
                                                         padding='same', regularizer='L2', reuse=reuse, scope='trans5',
                                                         weights_init=init)
            net6 = tflearn.layers.merge_ops.merge(
                [net6, tf.slice(net2, [0] + [(36 - 20) // 2] * 3 + [0], [-1] + [20] * 3 + [32]),
                 tf.slice(net2_2, [0] + [(36 - 20) // 2] * 3 + [0], [-1] + [20] * 3 + [32])], 'elemwise_sum',
                name='merge5')
            net6 = tflearn.layers.conv_3d(net6, 32, 3, activation='linear', padding='valid', regularizer='L2',
                                          reuse=reuse, scope='conv6-1', weights_init=init)
            net6 = tflearn.activation(net6, 'prelu')

        with tf.variable_scope("level7"):
            net7 = tflearn.layers.conv.conv_3d_transpose(net6, 16, 2, [36] * 3, strides=2, activation='prelu',
                                                         padding='same', regularizer='L2', reuse=reuse, scope='trans6',
                                                         weights_init=init)
            net7 = tflearn.layers.merge_ops.merge(
                [net7, tf.slice(net1, [0] + [(74 - 36) // 2] * 3 + [0], [-1] + [36] * 3 + [16]),
                 tf.slice(net1_2, [0] + [(74 - 36) // 2] * 3 + [0], [-1] + [36] * 3 + [16]),
                 tf.slice(net1_3, [0] + [(74 - 36) // 2] * 3 + [0], [-1] + [36] * 3 + [16])], 'elemwise_sum',
                name='merge6')
            net7 = tflearn.layers.conv_3d(net7, 16, 3, activation='linear', padding='valid', regularizer='L2',
                                          reuse=reuse, scope='conv7-1', weights_init=init)
            net7 = tflearn.activation(net7, 'prelu')

        with tf.variable_scope("out"):
            net_fc1 = tflearn.layers.conv_3d(net7, 16, 1, activation='linear', padding='valid', regularizer='L2',
                                             reuse=reuse, scope='fc1', weights_init=init)
            net_fc1 = tflearn.activation(net_fc1, 'prelu')
            net_out = tflearn.layers.conv_3d(net_fc1, 2, 1, activation='linear', padding='valid', regularizer='L2',
