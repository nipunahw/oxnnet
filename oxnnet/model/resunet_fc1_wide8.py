...
class Model(object):
    ...
    def build_feats_writer(self):
        ...
        return StandardFeatsWriter(segment_size_in_test, segment_size_out, crop_by, stride_test, self.batch_size, 8)
    ...
    def build_net(self, X, reuse=False, segment_size_in=segment_size_in,feats=None):
        ...
        with tf.variable_scope("level1"):
            net1 = tflearn.layers.conv_3d(X, 8, 3, activation='linear',padding='valid',regularizer='L2',reuse=reuse,scope='conv1-1', weights_init=init )
            net1 = tflearn.activation(net1, 'prelu')

        with tf.variable_scope("level2"):
            net2_in = tflearn.layers.conv_3d(net1, 16, 2, strides=2, activation='prelu',padding='valid',regularizer='L2',reuse=reuse,scope='ds1', weights_init=init)
            net2_in = tflearn.layers.normalization.batch_normalization(net2_in, reuse=reuse, scope='batch2-1')
            net2 = tflearn.layers.conv_3d(net2_in, 16, 3, activation='linear',padding='valid', regularizer='L2',reuse=reuse,scope='conv2-1', weights_init=init)
        ...
        with tf.variable_scope("level3"):
            net3_in = tflearn.layers.conv_3d(net2, 32, 2, strides=2, activation='prelu',padding='valid',regularizer='L2',reuse=reuse,scope='ds2', weights_init=init)
            net3_in = tflearn.layers.normalization.batch_normalization(net3_in, reuse=reuse, scope='batch3-1')
            net3 = tflearn.layers.conv_3d(net3_in, 32, 3, activation='linear',padding='valid',regularizer='L2',reuse=reuse,scope='conv3-1', weights_init=init)
        ...
        with tf.variable_scope("level4"):
            net4_in = tflearn.layers.conv_3d(net3, 64, 2, strides=2, activation='prelu',padding='valid',regularizer='L2',reuse=reuse,scope='ds3', weights_init=init)
            net4_in = tflearn.layers.normalization.batch_normalization(net4_in, reuse=reuse, scope='batch4-1')
            net4 = tflearn.layers.conv_3d(net4_in, 64, 3, activation='linear',padding='valid',regularizer='L2',reuse=reuse,scope='conv4-1', weights_init=init)
        ...
        with tf.variable_scope("level5"):
            net5 = tflearn.layers.conv.conv_3d_transpose(net4, 32, 2, [12]*3,strides=2, activation='prelu',padding='same',regularizer='L2',reuse=reuse,scope='trans2', weights_init=init)
            net5 = tflearn.layers.normalization.batch_normalization(net5, reuse=reuse, scope='batch5-1')
            net5 = tflearn.layers.merge_ops.merge([net5, tf.slice(net3,[0]+[(16-12)//2]*3+[0],[-1]+[12]*3+[32])],'elemwise_sum',name='merge5')
            net5 = tflearn.layers.conv_3d(net5, 32, 3, activation='linear',padding='valid',regularizer='L2',reuse=reuse,scope='conv5-1', weights_init=init)
        ...
        with tf.variable_scope("level6"):
            net6 = tflearn.layers.conv.conv_3d_transpose(net5, 16, 2, [20]*3,strides=2, activation='prelu',padding='same',regularizer='L2',reuse=reuse,scope='trans3', weights_init=init)
            net6 = tflearn.layers.normalization.batch_normalization(net6, reuse=reuse, scope='batch6-1')
            net6 = tflearn.layers.merge_ops.merge([net6, tf.slice(net2,[0]+[(36-20)//2]*3+[0],[-1]+[20]*3+[16])],'elemwise_sum',name='merge6')
            net6 = tflearn.layers.conv_3d(net6, 16, 3, activation='linear',padding='valid',regularizer='L2',reuse=reuse,scope='conv6-1', weights_init=init)
        ...
        with tf.variable_scope("level7"):
            net7 = tflearn.layers.conv.conv_3d_transpose(net6, 8, 2, [36]*3,strides=2, activation='prelu',padding='same',regularizer='L2',reuse=reuse,scope='trans4', weights_init=init)
            net7 = tflearn.layers.normalization.batch_normalization(net7, reuse=reuse, scope='batch7-1')
            net7 = tflearn.layers.merge_ops.merge([net7, tf.slice(net1,[0]+[(74-36)//2]*3+[0],[-1]+[36]*3+[8])],'elemwise_sum',name='merge9')
            net7 = tflearn.layers.conv_3d(net7, 8, 3, activation='linear',padding='valid',regularizer='L2',reuse=reuse,scope='conv9-1', weights_init=init)
        ...
        with tf.variable_scope("out"):
            net_fc1 = tflearn.layers.conv_3d(net7, 8, 1, activation='linear',padding='valid',regularizer='L2',reuse=reuse,scope='fc1', weights_init=init)
